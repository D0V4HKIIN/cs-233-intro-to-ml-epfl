...
class ShowPrints:
    """Enables normal printing for calling student code."""
...
no_print = ShowPrints  # HidePrints  # alias
...
class TestProject(unittest.TestCase):
    ...
    def test_3a_linear_regression(self):
        ...
        with no_print():
            pred_labels_train = linear_regression.fit(
    ...
    def test_3c_cross_validation(self):
        ...
        for i, (arr, shape) in enumerate(zip([train_data, train_label, val_data, val_label],
            ...
            with self.subTest(f"Checking output no {i}"):
                self.assertIsInstance(
                    arr, np.ndarray, f"cross_validation.splitting_fn() should output arrays, not {type(arr)}")
                self.assertEqual(
                    arr.shape, shape, f"cross_validation.splitting_fn() output has wrong shape ({arr.shape} != {shape})")
        with no_print():
            best_hyperparam, best_acc = cross_validation(
                method_obj, search_arg_name, search_args, data, labels, k_fold)
        self.assertIsInstance(best_hyperparam, (int, float),
                              f"cross_validation.cross_validation() output `best_hyperparam` should be float or int, not {type(best_hyperparam)}")
        self.assertIsInstance(
            best_acc, float, f"cross_validation.cross_validation() output `best_acc` should be float, not {type(best_acc)}")

        # Test on easy dummy data
        self.assertTrue(all([x not in train_data for x in val_data]),
                        f"cross_validation.splitting_fn() is not working on dummy data")
        self.assertEqual(
            best_hyperparam, 2, f"cross_validation.cross_validation() is not working on dummy data")
        self.assertTrue(np.isclose(
            best_acc, 1.), f"cross_validation.cross_validation() is not working on dummy data")
